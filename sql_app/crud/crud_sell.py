...
class CRUDSell(CRUDBase[Sell, SellCreate, SellInDBBase]):
    ...
    def get_unique_by_provider_email(self, db: Session, service_provider_email: str) -> Optional[SellInDBBase]:
        return db.query(self.model).filter(self.model.service_provider_name == service_provider_email).distinct(self.model.client_name).all()
